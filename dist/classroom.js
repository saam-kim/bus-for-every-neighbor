...
export function watchClass(code,callback){let stop=()=>{};connectBackend().then(()=>{if(!hasFirebase){const send=()=>callback(structuredClone(readDemo()[code]||null));send();channel.addEventListener('message',send);window.addEventListener('storage',send);stop=()=>{channel?.removeEventListener('message',send);window.removeEventListener('storage',send);};return;}stop=sdk.onValue(sdk.ref(db,`classes/${code}`),snap=>callback(snap.exists()?snap.val():null),err=>callback(null,err));});return()=>stop();}
...
export async function createClass(){const {uid}=await connectBackend();for(let i=0;i<20;i++){const code=String(Math.floor(100000+Math.random()*900000));if(await readClass(code))continue;const value=initialClass(code,uid);if(!hasFirebase)await demoMutate(code,()=>value);else await sdk.set(sdk.ref(db,`classes/${code}`),value);localStorage.setItem(`bus-teacher-${code}`,uid);return value;}throw new Error('수업 코드를 만들지 못했습니다. 잠시 후 다시 시도해 주세요.');}
...
export async function leaveTeam(code,teamId){const {uid}=await connectBackend(),klass=await readClass(code);if(uid!==teamId&&klass?.teacherUid!==uid)throw new Error('모둠을 나갈 권한이 없습니다.');if(!hasFirebase)return demoMutate(code,v=>{const teams={...(v?.teams||{})};delete teams[teamId];return {...v,teams};});await sdk.remove(sdk.ref(db,`classes/${code}/teams/${teamId}`));localStorage.removeItem(`bus-team-${code}`);}
